...
impl TryFrom<[i16; 3]> for Color {
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
        let u8_result: Vec<u8> = arr
            .iter()
            .map(|nr| <i16 as TryInto<u8>>::try_into(*nr).map_err(|_| Self::Error::IntConversion))
            .collect::<Result<Vec<u8>, Self::Error>>()?;
        let [red, green, blue]: [u8; 3] = u8_result.try_into().unwrap();
        Ok(Color { red, green, blue })
    }
}
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        <Color as TryFrom<[i16; 3]>>::try_from(slice.try_into().map_err(|_| Self::Error::BadLen)?)
    }
}
